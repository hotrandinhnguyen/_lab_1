...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Keep only the last 3 turns (3 user + 3 assistant = 6 messages)
        history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = f"{'Prompt':<42} | {'GPT-4o Response':<42} | {'Mini Response':<42} | {'GPT-4o Latency':>16} | {'Mini Latency':>14}"
    ...
    for r in results:
        row = (
            ...
            f"{r.get('gpt4o_latency', 0):>16.3f} | "
            f"{r.get('mini_latency', 0):>14.3f}"
        )
        rows.append(row)
    return "\n".join(rows)
